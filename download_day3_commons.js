...
const targets = [
    { folder: 'Quattro-Canti', prefix: 'quattrocanti', query: 'Quattro Canti Palermo', expected: 3 },
    { folder: 'Teatro-Massimo', prefix: 'massimo', query: 'Teatro Massimo Palermo', expected: 3 },
    { folder: 'Palermo-Cathedral', prefix: 'cathedral', query: 'Palermo Cathedral exterior', expected: 3 }
];

async function fetchCommonsImages(query, limit = 10) {
    const url = `https://commons.wikimedia.org/w/api.php?action=query&generator=search&gsrsearch=${encodeURIComponent(query)}&gsrnamespace=6&gsrlimit=50&prop=imageinfo&iiprop=url&format=json`;
...
async function processTargets() {
    ...
    for (const target of targets) {
        ...
        let allUrls = await fetchCommonsImages(target.query, 10);
